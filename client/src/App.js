...
function App() {
  ...
  useEffect(()=>{
    ...
    axios.post(`http://${process.env.REACT_APP_API_IP}:${process.env.REACT_APP_API_PORT}/api/fqdn/all`, {})
      .then(res=>{
        ...
        try {
          ...
          importedData.forEach((importedFqdn) => {
            ...
            if (existingIndex === -1) {
              ...
              axios.post(`http://${process.env.REACT_APP_API_IP}:${process.env.REACT_APP_API_PORT}/api/fqdn/new`,importedFqdn)
            } else {
              setFqdns((prevData) => {
              ...
              axios.post(`http://${process.env.REACT_APP_API_IP}:${process.env.REACT_APP_API_PORT}/api/fqdn/update`,importedFqdn)
            }
          });
        } catch (error) {
  ...
  const deleteFqdn = () => {
    ...
    axios.post(`http://${process.env.REACT_APP_API_IP}:${process.env.REACT_APP_API_PORT}/api/fqdn/delete`, fqdnToDelete)
      .then(res => {
  ...
  const runWildfire = () => {
    ...
    if (!scanSingleDomain){
      ...
      axios.post(`http://${process.env.REACT_APP_TOOLKIT_IP}:${process.env.REACT_APP_TOOLKIT_PORT}/wildfire`, payload)
      .then(res => {
        setScanRunning(true);
  ...
  const handleCollectScreenshotsButton = () => {
    axios.post(`http://${process.env.REACT_APP_TOOLKIT_IP}:${process.env.REACT_APP_TOOLKIT_PORT}/collect_sceenshots`,{})
      .then(res => {

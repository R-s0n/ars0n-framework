...
def full_nuclei_scan(args, now):
    try:
        ...
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates -l /tmp/urls.txt -stats -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
...
def technologies_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/technologies -l /tmp/urls.txt -stats -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def misconfiguration_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/http/misconfiguration -l /tmp/urls.txt -timeout 7 -vv -stats -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def cves_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        protonvpn_necessary(args, "Misconfigurations")
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/http/cves -l /tmp/urls.txt -stats -timeout 7 -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def cnvd_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        protonvpn_necessary(args, "Misconfigurations")
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/http/cnvd -l /tmp/urls.txt -stats -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def exposed_panels_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/http/exposed-panels -l /tmp/urls.txt -stats -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def exposures_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        protonvpn_necessary(args, "Misconfigurations")
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/http/exposures -l /tmp/urls.txt -stats -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def miscellaneous_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        protonvpn_necessary(args, "Misconfigurations")
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/http/miscellaneous -l /tmp/urls.txt -stats -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def network_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        protonvpn_necessary(args, "Misconfigurations")
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/http/osint -l /tmp/urls.txt -stats -vv -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def file_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        protonvpn_necessary(args, "Misconfigurations")
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/http/default-logins -l /tmp/urls.txt -vv -stats -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def dns_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        protonvpn_necessary(args, "Misconfigurations")
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/dns -l /tmp/urls.txt -stats -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def vulnerabilities_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        protonvpn_necessary(args, "Misconfigurations")
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/http/vulnerabilities -l /tmp/urls.txt -stats -timeout 7 -vv -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def rs0n_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        protonvpn_necessary(args, "Misconfigurations")
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t ./custom -l /tmp/urls.txt -stats -vv --headless -sb -hbs 10 -headc 1 -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def headless_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        protonvpn_necessary(args, "Misconfigurations")
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/headless -l /tmp/urls.txt -stats -vv --headless -sb -hbs 10 -headc 1 -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)
...
def ssl_nuclei_scan(args, now, logger):
    ...
    try:
        ...
        protonvpn_necessary(args, "Misconfigurations")
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/nuclei -t {home_dir}/nuclei-templates/ssl -l /tmp/urls.txt -stats -fr -fhr -hm -o /tmp/{args.fqdn}-{now}.json -jsonl"], shell=True)
        data = process_results(args, now)
        thisFqdn = get_fqdn_obj(args)

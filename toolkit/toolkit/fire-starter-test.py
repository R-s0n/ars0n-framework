...
def parse_amass_file(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            line = line.strip()
            asn_match = re.match(r'AS\d+', line)
            if asn_match:
                asns.append(asn_match.group())
            ...
            ipv6_match = re.match(r'[0-9a-fA-F:]+', line)
            if ipv6_match:
                ipv6_addresses.append(ipv6_match.group())
    return {
        "asns": asns,
...
def get_ips_from_amass(thisFqdn):
    file_path = "./temp/amass.tmp"
    result = parse_amass_file(file_path)
    print("ASNs:", result["asns"])
    thisFqdn['asns'] = result["asns"]
    print("CIDR Subnets:", result["cidr_subnets"])
    thisFqdn['subnets'] = result["cidr_subnets"]
    print("IPv4 Addresses:", result["ipv4_addresses"])
    print("IPv6 Addresses:", result["ipv6_addresses"])
    for ip_address in result["ipv4_addresses"]:
        exists = False
        for ip_obj in thisFqdn['ips']:
            if ip_address.strip() == ip_obj['ip']:
                exists = True
        if exists == True:
            data = {
                "ip": ip_address,
            ...
            thisFqdn['ips'].append(json.dumps(data))
    for ip_address in result["ipv6_addresses"]:
        exists = False
        for ip_obj in thisFqdn['ips']:
            if ip_address.strip() == ip_obj['ip']:
                exists = True
